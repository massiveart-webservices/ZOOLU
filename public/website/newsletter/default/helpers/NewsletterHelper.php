    ...
    if($objMyMultiRegion instanceof GenericElementRegion){
      foreach($objMyMultiRegion->RegionInstanceIds() as $intRegionInstanceId){
        ...
        if($strBlockTitle != '' || $strBlockDescription != ''){
          if($strBlockTitle != '') $strTextBlock .= '<tr><td><h3>'.$strBlockTitle.'</h3></td></tr>';
          ...
          if($objFiles != '' && count($objFiles) > 0) $strHtmlOutputContent .= '<td class="description">'.$strBlockDescription.'</td>';
          else $strHtmlOutputContent .= '<td colspan="2" class="description">'.$strBlockDescription.'</td>';
