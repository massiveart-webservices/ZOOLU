...
Massiveart.Form = Class.create({
    ...
    addMarker: function (area, fieldId, x, y, targetregion) {
        ...
        var id = 1;
        if (instances != '') {
            instances.scan(/\[\d*\]/, function (widgets) {
    ...
    addRegionInstancesToSelect: function (fieldId, targetregion, regionUniqueId) {
        ...
        $$('#' + fieldId + '_regionSelect option:not(.default)').each(function (e) {
            e.remove();
        });
        var i = 1;
     ...
     */
    getAddContactOverlay: function (areaId, fieldname) {
        $(this.updateOverlayContainer).innerHTML = '';
        myCore.putCenter('overlayGenContentWrapper');
        $('overlayGenContentWrapper').show();
        if ($(areaId)) {
     ...
     * searchContactEnter
     */
    searchContactEnter: function (event, fieldname) {
        if (event.keyCode == Event.KEY_RETURN) {
            this.searchContact(fieldname);
        }
    },
     ...
     * searchContact
     */
    searchContact: function (fieldname) {
        var fieldId = $(fieldname).readAttribute('fieldid');
        ...
        new Ajax.Updater(this.updateOverlayContainer, '/zoolu/cms/overlay/contact', {
            ...
            parameters: {
                ...
                groupIds: $(fieldname).value,
                fieldId: fieldId,
                search: $('contactSearch_' + fieldId).value
            },
            onComplete: function () {
                myCore.putOverlayCenter('overlayGenContentWrapper');
            }
     ...
     * resetContactSearch
     */
    resetContactSearch: function (fieldname) {
        var fieldId = $(fieldname).readAttribute('fieldid');
        $('contactSearch_' + fieldId).setValue('');
        this.searchContact(fieldname);
    },
...
//      }
    ...
    addRegion: function (regionId) {
        ...
        var arrWidgets = [];
        $('Region_' + regionId + '_Instances').value.scan(/\[\d*\]/, function (widgets) {
            arrWidgets.push(widgets[0].gsub(/\[/, '').gsub(/\]/, ''))
        });
...
//      });
    ...
    initTag: function (elementId, autocompleteFeed) {
        if ($(elementId) && $(elementId + '_autocompleter')) {
            ...
            autocompleteFeed.each(function (t) {
                tagList.autoFeed(t)
            });
        }
    },
