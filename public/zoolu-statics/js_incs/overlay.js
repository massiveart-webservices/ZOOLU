...
Massiveart.Overlay = Class.create({
    ...
    getRootNavItem: function (rootLevelId, viewtype) {
        ...
        $$('.olnavigationwrapper .img_folder_on_open').each(function (element) {
            element.removeClassName('img_folder_on_open');
        });
        $$('.olnavigationwrapper .olsubnav').each(function (element) {
            element.hide();
        });
    ...
    close: function (id) {
        ...
        if (this.overlayCounter > 1 && id != undefined) {
            ...
            // field overlays
            $$('.fieldOverlayWrapper').each(function (elem) {
                elem.hide();
            });
            //this.lastFolderId = null;
            this.overlayCounter = 0;
