    ...
    */
    ...
    private function sendMail($blnSpecialForm = false){
        ...
        if(count($this->arrMailRecipients) > 0){
            ...
            $counter = 0;
            $allRecipientsEmail = str_replace(" ", "", $this->arrMailRecipients['Email']);
            $allRecipientsEmail = str_replace(",", ";", $allRecipientsEmail);
            $allRecipientsName = str_replace(array(" ; ", " , ", "; ", ", ", ","), ";", $this->arrMailRecipients['Name']);
            $allRecipientsEmail = explode(';', $allRecipientsEmail);
            $allRecipientsName = explode(';', $allRecipientsName);

            if (count($allRecipientsEmail) == count($allRecipientsName)) {
                foreach ($allRecipientsEmail as $recipientEmail) {
                    $mail->addTo($allRecipientsEmail[$counter], $allRecipientsName[$counter]);
                    $counter++;
                }
            } else {
                if ($allRecipientsEmail[0] != '') {
                    $mail->addTo($allRecipientsEmail[0], $allRecipientsName[0]);
                }
            }
            ...
            if ($value != '') {
                ...
                if ($key == 'idRootLevels' || $key == 'idPage' || $key == 'subject' || $key == 'blnDynForm' || preg_match('/type$/', $key) || preg_match('/other$/', $key)) {
                    ...
                    if ($blnDynForm) {
                        ...
                        if ($arrKey[0] == $strCountryKey) {
                            $this->loadCountryById($value);
                            $value = $this->arrFormDataReplacer['country'];
                        }
                        
                        $strHtmlBody .= '<strong>' . str_replace('--', ' ', urldecode($arrKey[0])) . ':</strong> ' . $value . '<br />';
                    } else {
                        if ($arrKey[0] == $strCountryKey) {
                            $this->loadCountryById($value);
                            $value = $this->arrFormDataReplacer['country'];
     ...
     * @version 1.0
     */
    private function insertDatabase($strDB = '', $blnGlobal = false){
        ...
        if(count($this->arrFormData) > 0){
            if(isset($this->core->config->mail->database) && $this->core->config->mail->database != ''){
                ...
                if (key_exists('privacy-policy', $arrFormData)) {
                    if ($arrFormData['privacy-policy'] == 'on') {
                        $arrFormData['privacy-policy'] = 1;
                    } else {
                        $arrFormData['privacy-policy'] = 0;
                    }
                }
                if (key_exists('checkLegalnotes', $arrFormData)) {
                    if ($arrFormData['checkLegalnotes'] == 'on') {
                        $arrFormData['checkLegalnotes'] = 1;
                    } else {
                        $arrFormData['checkLegalnotes'] = 0;
                    }
                }
                if ($blnGlobal) {
                    $arrTableData['idGlobals'] = $arrFormData['idPage'];
                } else {
                ...
                if ($this->arrFormData['blnDynForm'] == 'true') {
                    $arrTableData['content'] = str_replace(array('--', '\r\n'), array(' ', ' '), urldecode($this->convertFormData($arrFormData)));
                } else {
                    unset($arrFormData['idPage']);
                    unset($arrFormData['sname_type']);
                    unset($arrFormData['fname_type']);
                    unset($arrFormData['email_type']);


                    $arrTableData = $arrFormData;
     ...
     * @return string
     */
    private function loadCountryById($id)
    {
        $objSelect = $this->getModelGenericData()->getGenericTable('categories')->select();
        $objSelect->setIntegrityCheck(false);
        $objSelect->from('categories', array('id'));
        $objSelect->join('categoryTitles', 'categoryTitles.idCategories = categories.id AND categoryTitles.idLanguages = ' . $this->core->intLanguageId, array('title'));
        $objSelect->where('categories.id = ?', $id);
        $objResult = $this->getModelGenericData()->getGenericTable('categories')->fetchAll($objSelect);
        $objData = $objResult->current();
        $this->arrFormDataReplacer['country'] = $objData->title;
    }
