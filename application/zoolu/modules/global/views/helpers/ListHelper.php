...
class ListHelper {
  ...
  function getList($objPaginator, $strOrderColumn, $strSortOrder, $strSearchValue) {
    ...
    foreach ($objPaginator as $objItem) {
      ...
      $arrItem['title'] .= ' ('.$arrItem['languageCodes'].')';
      $strLanguageCodes = $arrItem['languageCodes'];
      unset($arrItem['languageCodes']);
  ...
  public function getSearchResultList($objRowset, $strSearchValue){
    ...
    if($objRowset instanceof Zend_Db_Table_Rowset_Abstract && count($objRowset) > 0){
      ...
      foreach($objRowset as $objRow){
        $intCounter++;
        $strOutput .= '<li class="modulus'.($intCounter % 2).'"><a href="#" onclick="myGlobal.addElementLink(\''.$objRow->globalId.'\'); return false;">'.htmlentities($objRow->title, ENT_COMPAT, $this->core->sysConfig->encoding->default).' ('.$objRow->glLanguages.')</a></li>';
      }
      $strOutput .= '</ul>';
    }else{
