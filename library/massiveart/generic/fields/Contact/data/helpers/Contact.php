...
class GenericDataHelper_Contact extends GenericDataHelperAbstract  {
    ...
    try{
      ...
      foreach($arrValues as $strValue){
        if(!empty($strValue)){
          ...
          $this->getModelGenericData()->getGenericTable($strType.'-'.$this->objElement->Setup()->getFormId().'-'.$this->objElement->Setup()->getFormVersion().'-Region'.$objRegion->getRegionId().'-InstanceContacts')->insert($arrFileData);
        }
      }
    }catch(Excpetion $exc){
      $this->core->logger->err($exc);
  ...
  public function loadInstanceData($strType, $strElementId, $objRegion, $intVersion){
    try{
      $objGenTable = $this->getModelGenericData()->getGenericTable($strType.'-'.$this->objElement->Setup()->getFormId().'-'.$this->objElement->Setup()->getFormVersion().'-Region'.$objRegion->getRegionId().'-InstanceContacts');
      
      $objSelect = $objGenTable->select();
      $objSelect->setIntegrityCheck(false);
      $objSelect->from($objGenTable->info(Zend_Db_Table_Abstract::NAME), array('id', 'idContacts', 'idRegionInstances'));
      ...
      foreach($arrRawInstanceData->toArray() as $arrInstanceDataRow){
        $arrInstanceData[$arrInstanceDataRow['sortPosition']][] = $arrInstanceDataRow['idContacts'];
      ...
      $this->core->logger->debug($arrInstanceData);
